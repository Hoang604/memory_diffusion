...
class MyTrainingStats(TrainingStats):
    critic_loss: float
    overall_loss: float
...
class DiffusionOPT(BasePolicy):
    ...
    def update(
        ...
        """
        Update the policy using a batch of transitions from the replay buffer.
        Args:
            sample_size (int): Number of samples to draw from the replay buffer.
            buffer (ReplayBuffer): Replay buffer containing the transitions.
            **kwargs: Additional arguments for the update process.
        Returns:
            TrainingStats: A dictionary containing the training statistics.
        """
        # If no replay buffer is provided, return an empty dictionary
        if buffer is None: return {}
        self.updating = True # Indicate that the policy is being updated (required by the base class)
        ...
        # learn() returns a dict of losses
        loss_dict = self.learn(batch, **kwargs)
        ...
        result = MyTrainingStats(critic_loss=loss_dict['loss/critic'], overall_loss=loss_dict['overall_loss'])
        return result
    def forward(
            self,
            batch: Batch,
    ...
    def _update_bc(self, batch: Batch, update: bool = False) -> torch.Tensor:
        ...
        obs_ = to_torch(batch.obs, device=self._device, dtype=torch.float32)
        # expert_actions = torch.Tensor([info["sub_optimal_action"] for info in batch.info]).to(self._device)
